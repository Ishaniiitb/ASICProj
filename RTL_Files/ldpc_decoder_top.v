...
module ldpc_decoder_top (
  ...
  always @(*) begin
    hd_mux = 1'b0;
    for (hd_i = 0; hd_i < `N_COLS; hd_i = hd_i + 1)
      if (hd_col == hd_i[`COL_AW-1:0])
        hd_mux = cv_data_arr[hd_i];
  end
  assign hd_data = hd_mux;

  // ---- Parity aggregation -------------------------------------------
  reg parity_acc;
  integer pc_c, pc_v, pc_l;
  always @(*) begin
    parity_acc = 1'b0;
    for (pc_c = 0; pc_c < `N_COLS; pc_c = pc_c + 1)
      for (pc_v = 0; pc_v < `Q; pc_v = pc_v + 1)
        for (pc_l = 0; pc_l < `N_LAYERS; pc_l = pc_l + 1)
          if (layer_active[pc_l])
            parity_acc = parity_acc |
              msg_bus_flat[pc_c][(pc_v*`N_LAYERS + pc_l)*`MSG_W + `PC_BIT];
  end
  wire parity_ok = ~parity_acc;
